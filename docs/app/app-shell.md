...
- `ui.tsx` — shared: the `panel`/`kicker`/`btnPrimary`/`input`/`label` class strings, `scoreTone`, `useRoute` + the `*Href` builders, `UserLink`/`CategoryLink`, `ReviewText` (hashtag linkifier), `ReviewQuote` (the clamped quote + "more" expander), `Tag`/`profileTags`, `Heart`, `LoadError`, and **`RankingRow`**.
  - **`RankingRow` is the one row implementation.** It was three near-identical ~40-line blocks (board panel, activity feed, profile page) differing only in their headline, so every mobile fix had to be applied three times by hand. It takes a `headline` slot, a container `className`, and `controls` (`'none'` for the activity feed, which doesn't reserve the owner column; `'owner'` elsewhere). Delete confirmation, the inline editor and the review expander all live inside it — add row behaviour here, not at a call site.
...
- `text.ts` — the pure helpers (`timeAgo`, `HASHTAG_RE`, `hashtagsIn`), split out of ui.tsx so `text.test.ts` can import them without pulling in React or the Supabase client (which builds a realtime client at module load and needs a WebSocket). New pure logic worth testing goes here.
