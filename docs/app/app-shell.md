...
- **The board's categories/activity switch is a real tablist** (`role="tablist"`/`tab`/`tabpanel`, `aria-selected`, arrow keys). The Top-rated/A–Z control next to it is a **radiogroup**, not a tablist — it picks an ordering rather than switching panels, and `role="tab"` without a tabpanel would announce something untrue.
