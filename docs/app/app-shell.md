...
- **Mobile:** the page is a single centered column that already reflows (stat tiles are `grid-cols-2 sm:grid-cols-4`, etc.). The one non-obvious idiom is the **ranking row**: on phones it stacks (name + review on line one, then time/stars/score/heart/delete on line two) so the content isn't crushed to a few characters, while on `sm+` it collapses back to a single line via `sm:contents` on the meta wrapper — so desktop stays pixel-identical. Within the name line, the **username gets display priority** (`shrink-0`, never truncates) and the food shortens to fill the rest — so the left column reads as who ranked, owner-ruled. Owner-only controls that reveal on hover (the delete ✕) must also be visible without hover on touch (`opacity-100 sm:opacity-0 sm:group-hover:opacity-100`), or the feature is unreachable on mobile. The header hides the rankings/categories count below `sm`.
