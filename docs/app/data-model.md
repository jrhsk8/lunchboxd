...
- **`profiles`** — `id` (FK to `auth.users`, cascade), `username` (**citext** unique, 2–24 chars, `^[A-Za-z0-9_-]+$`, and not one of the reserved names), `created_at`, `is_admin`, `is_supporter`, `banned_at`, `tags` (text[], constrained to the roster `peloton`/`zwift`/`runner`). Created by the `handle_new_user` trigger; see [auth.md](auth.md). The update grant is **column-scoped to `username, tags, card_slot_1, card_slot_2, card_slot_3, card_accent`** — six columns, widened from two when the calling card shipped (`20260725022000_calling_card.sql`). That scoping is the entire defence against somebody setting their own `is_admin` or clearing `banned_at`; widen it only to a column that is safe to self-issue, and never back to whole-row. The update policy also requires `banned_at is null`, so a banned account can't cycle handles or re-add the flair the ban cleared. A `before update of username` trigger (`profiles_handle_rules`) adds the two rules RLS can't express per-column: a guest can't rename until it has an email, and nobody can claim a `guest-*` name. See [auth.md](auth.md).
...
Three stats a person picks about themselves, stored on their profile row and read through a view.

**Storage** (`20260725022000_calling_card.sql`): `card_slot_1`, `card_slot_2`, `card_slot_3` and `card_accent` on `profiles`, each pinned by a CHECK constraint — the three slots to the fourteen-key stat vocabulary, the accent to the five swatches. Null slots mean nobody has opened the studio, and the client falls back to the default trio per position rather than rendering blank. The update grant covers exactly these four columns alongside `username` and `tags` (§ Tables), which is why the studio can save without ever being able to reach `is_admin`.

**`profile_card_stats`** (view, security_invoker) — one row per profile carrying every number any of the fourteen keys can show: the counters, the lifetime average, and the five named stats (best and worst food, most-ranked category, kindest and harshest category) each as a name and a value. Its own view rather than more columns on `profile_stats` because it is a pile of per-person aggregates that only the card needs, and because the Eaters tab reads the same shape for many people at once — `eaters` is built on it (§ The Eaters view).

Two rules live in the SQL rather than the client. **Kindest and harshest need at least two rankings in a category**, or a single 5.0 crowns a category somebody has barely eaten in. And a **named stat is null unless both halves are present** — a name with no score is a bug, not an absence, and `cardStatsFrom` (`src/calling-card.ts`, tested) drops it rather than rendering half of it.

**The accent is resolved on read**, not on write (`resolveAccent`): a supporter whose badge lapses keeps their stored choice and simply stops rendering it, and gets it back if the badge returns. Nothing has to be cleaned up when a supporter lapses, which is the point.
